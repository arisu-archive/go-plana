...
// computeHash calculates the CRC32 hash of the data using the provided initial value.
func computeHash(data []byte, iv uint32) uint32 {
	for _, b := range data {
		//nolint:mnd // Well known algorithm and constants
		iv = (iv << 8) ^ CRC32Table[(iv>>24)^uint32(b)]
	}
	return iv
}
...
func (*Processor) BuildPacket(payload []byte, checksum, encodedProtocol uint32, key *UserSession) []byte {
	...
	packet.Write(checksumBytes)

	// Protocol header
	// Convert uint32 to bytes in little-endian and write to packet
	protocolHeader := make([]byte, 4)
	binary.LittleEndian.PutUint32(protocolHeader, encodedProtocol)
